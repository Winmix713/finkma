...
import { validateFigmaUrl, type FigmaUrlValidation } from "@/utils/figma-url-validator"
...
export function useFigmaConnection(options: UseFigmaConnectionOptions = {}) {
  const { apiKey, enableRetry = true, maxRetries = 3, retryDelay = 1000, timeout = 10000 } = options
  ...
  const createApiError = (status: number, message: string, details?: string): FigmaApiError => {
    ...
    switch (status) {
      case 401:
        ...
        break
      case 403:
        userMessage = "Access denied. You don't have permission to access this file."
        break
      case 404:
        userMessage = "Figma file not found. Please check the URL and ensure the file exists."
        break
      case 429:
      ...
      case 502:
      case 503:
        userMessage = "Figma API is temporarily unavailable. Please try again later."
        break
      default:
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes("fetch")) {
          throw createApiError(0, "Network error. Please check your internet connection.")
        }
      }
      ...
      throw createApiError(500, error instanceof Error ? error.message : "Unknown error occurred")
    }
  }
  ...
  const processUrl = useCallback(
    async (figmaUrl: string): Promise<ProcessingResult> => {
      ...
      if (!validation.isValid) {
        const error = createApiError(400, validation.error || "Invalid Figma URL")
        return {
          success: false,
          error,
          url: figmaUrl,
