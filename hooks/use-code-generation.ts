...
import type { FigmaApiResponse, GeneratedComponent, AppError, Toast } from "@/types/figma"
import {
  AdvancedCodeGenerator,
  type CodeGenerationOptions,
  type CustomCodeInputs,
} from "@/services/advanced-code-generator"
import { copyToClipboard, downloadFile, validateJSX, validateCSS, generateId } from "@/lib/utils"
...
export function useCodeGeneration(figmaData: FigmaApiResponse) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (components.length === 0) {
        throw new Error("No components could be generated from the provided Figma data")
      }

      setGeneratedComponents(components)
      setSelectedComponent(components[0])
      showSuccessToast(`${components.length} component${components.length > 1 ? "s" : ""} generated successfully!`)
      ...
      console.log("Code generation success:", {
        ...
        hasCustomCode: !!(customCode.jsx || customCode.css || customCode.cssAdvanced),
      })
    } catch (error) {
      ...
      const appError: AppError = {
        type: "generation",
        message: error instanceof Error ? error.message : "Unknown error occurred during code generation",
  ...
  }, [figmaData, options, customCode, validateInputs, showSuccessToast, showErrorToast])
