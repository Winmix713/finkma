...
export function useDesignTokens(options: UseDesignTokensOptions = {}) {
      ...
      const updatedTokens = state.collection.tokens.map((token) =>
        token.id === tokenId
          ? {
              ...token,
              ...updates,
              metadata: { ...token.metadata, updatedAt: new Date() },
            }
          : token,
      );
  ...
  const deleteToken = useCallback(
    (tokenId: string) => {
      ...
      const updatedTokens = state.collection.tokens.filter(
      ...
      setState((prev) => ({
        ...
        selection: {
          ...prev.selection,
          selectedTokens: new Set(
            [...prev.selection.selectedTokens].filter((id) => id !== tokenId),
          ),
          lastSelected:
            prev.selection.lastSelected === tokenId
              ? null
              : prev.selection.lastSelected,
        },
      }));
  ...
  const duplicateToken = useCallback(
    (tokenId: string) => {
      ...
      const originalToken = state.collection.tokens.find(
        (token) => token.id === tokenId,
      );
  ...
  const addToHistory = useCallback(
    (action: string, collection: TokenCollection) => {
      setState((prev) => {
        ...
        const newEntries = [
          ...prev.history.entries.slice(0, prev.history.currentIndex + 1),
          newEntry,
    ...
    return tokens.filter((token) => {
      ...
      if (filter.tags && filter.tags.length > 0) {
        const hasMatchingTag = filter.tags.some((tag) =>
          token.metadata.tags.includes(tag),
        );
        ...
        switch (filter.validationStatus) {
          ...
          case "invalid":
            if (
              ...
              token.validation.errors.length === 0
            )
              return false;
            break;
          case "warning":
