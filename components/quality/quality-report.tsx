...
import { AlertTriangle, CheckCircle, Info, ChevronDown, TrendingUp, Target, Lightbulb } from "lucide-react"
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Comprehensive analysis of your design quality and best practices</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            {/* Overall Score */}
            <div className="text-center space-y-4">
              <div className="space-y-2">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className={`text-4xl font-bold ${getScoreColor(qualityMetrics.overall)}`}>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className={`text-sm font-bold ${getScoreColor(qualityMetrics.accessibility)}`}>
                      {qualityMetrics.accessibility}
                    </span>
                  </div>
                </div>
                <Progress value={qualityMetrics.accessibility} className="h-2" />
                <p className="text-xs text-muted-foreground">WCAG compliance and inclusive design practices</p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className={`text-sm font-bold ${getScoreColor(qualityMetrics.performance)}`}>
                      {qualityMetrics.performance}
                    </span>
                  </div>
                </div>
                <Progress value={qualityMetrics.performance} className="h-2" />
                <p className="text-xs text-muted-foreground">Optimization and rendering efficiency</p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className={`text-sm font-bold ${getScoreColor(qualityMetrics.maintainability)}`}>
                      {qualityMetrics.maintainability}
                    </span>
                  </div>
                </div>
                <Progress value={qualityMetrics.maintainability} className="h-2" />
                <p className="text-xs text-muted-foreground">Code structure and design system consistency</p>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{qualityMetrics.issues.length} issues found across your design</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <Collapsible key={category}>
                  ...
                  <CollapsibleContent>
                    <div className="space-y-2 p-3 pt-0">
                      {issues.map((issue) => (
                        <div key={issue.id} className="flex items-start gap-3 p-3 rounded-md bg-muted/50">
                          {getSeverityIcon(issue.severity)}
                          <div className="flex-1 space-y-1">
                            <p className="text-sm font-medium">{issue.message}</p>
                            {issue.nodeId && <p className="text-xs text-muted-foreground">Node: {issue.nodeId}</p>}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Actionable suggestions to improve your design quality</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {qualityMetrics.issues
              ...
              .slice(0, 5)
              .map((issue) => (
                <div key={issue.id} className="flex items-start gap-3 p-3 rounded-md border">
