...
import { Users, Share2, UserPlus, Settings, Eye, Edit, Trash2 } from "lucide-react"
...
export function TeamWorkspace({ results, userInfo, onShare }: TeamWorkspaceProps) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold">{teamMembers.filter((m) => m.role === "viewer").length}</div>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold">
              {teamMembers.filter((m) => ["editor", "admin", "owner"].includes(m.role)).length}
            </div>
              ...
              {results.slice(0, 5).map((result) => (
                <div key={result.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div>
                    ...
                    <div className="text-sm text-gray-600">{Object.keys(result.data.components).length} components</div>
                  </div>

                  <Button variant="outline" size="sm" onClick={() => shareFile(result.id, result.data.name)}>
                ...
                <div key={file.fileId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      Shared with {file.sharedWith.length} members • {file.sharedAt.toLocaleDateString()}
                    </div>
                  </div>

                  <div className="flex items-center space-x-2">
                    <Badge variant="outline">{file.sharedWith.length} members</Badge>
