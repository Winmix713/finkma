...
export function FigmaUrlForm({ onSubmit, isProcessing = false, disabled = false }: FigmaUrlFormProps) {
  const [url, setUrl] = useState("")
  const [validation, setValidation] = useState<ReturnType<typeof validateFigmaUrl> | null>(null)
  ...
  const getValidationMessage = () => {
    ...
    if (validation.isValid) {
      return <p className="text-sm text-green-600 mt-1">Valid {validation.urlType} URL detected</p>
    } else {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Enter a Figma file URL to analyze and generate code</CardDescription>
      </CardHeader>
      <CardContent>
