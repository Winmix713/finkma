...
export class AdvancedCodeGenerator {
  ...
  private processNode(node: FigmaNode, index: number): GeneratedComponent | null {
    const componentName = this.sanitizeComponentName(node.name || `Component${index + 1}`)

    const jsx = this.generateJSX(node)
    const css = this.generateCSS(node)
    const typescript = this.options.typescript ? this.generateTypeScript(node) : undefined
    ...
    const metadata: ComponentMetadata = {
      ...
      figmaNodeId: node.id,
      hasCustomCode: !!(this.customCode?.jsx || this.customCode?.css || this.customCode?.cssAdvanced),
    }
...
export type ${componentName}Size = 'sm' | 'md' | 'lg'`
  ...
  private generateNodeContent(node: FigmaNode): string {
    ...
    if (node.children && node.children.length > 0) {
      return node.children.map((child) => this.generateNodeContent(child)).join("\n      ")
    }
    ...
    if (node.effects && node.effects.length > 0) {
      node.effects.forEach((effect) => {
        if (effect.type === "DROP_SHADOW" && effect.visible) {
          ...
          styles.push(`box-shadow: ${offsetX}px ${offsetY}px ${blur}px rgba(0, 0, 0, 0.1)`)
        }
      })
    }
...
@media (max-width: 480px) {
  ...
  private determineComponentType(node: FigmaNode): ComponentMetadata["componentType"] {
  ...
  private analyzeAccessibility(jsx: string, css: string): AccessibilityReport {
    ...
    if (jsx.includes("<button") && !jsx.includes("aria-label")) {
      suggestions.push("Consider adding aria-label to buttons for better screen reader support")
    }

    // Check for color contrast (simplified)
    const hasGoodContrast = css.includes("color:") && css.includes("background")
    ...
    return {
      ...
      hasAriaLabels: jsx.includes("aria-label") || jsx.includes("aria-labelledby"),
...
${this.customCode?.cssAdvanced ? `\n/* Advanced Custom CSS */\n${this.customCode.cssAdvanced}` : ""}`
  ...
  children?: React.ReactNode
      ...
      responsive: {
        ...
        flexboxUsed: false,
        gridUsed: false,
        mediaQueries: this.options.responsive ? ["@media (max-width: 768px)"] : [],
      },
      performance: {
