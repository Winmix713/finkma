...
describe("useFileDownload", () => {
  ...
  test("downloads with extension", async () => {
    ...
    await act(async () => {
      await result.current.downloadWithExtension("test content", "test", ".jsx")
    })
...
describe("useTabConfig", () => {
  ...
  test("returns available tabs", () => {
    ...
    expect(result.current.availableTabs.some((tab) => tab.id === "tsx")).toBe(true)
    expect(result.current.availableTabs.some((tab) => tab.id === "jsx")).toBe(false) // Hidden when TS available
  })
